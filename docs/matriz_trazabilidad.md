...
| `PASS WOKWI FINAL` | Verificado sobre el commit final identificado en el manifest de validacion. |
...
| RS-10, RS-11: startup fail-safe | `docs/logica_critica.md` y `docs/arquitectura_sistema.md` | `BootGuard`, `SYSTEM_STARTUP` y timeout inicial | SAFE-BOOT-01, SAFE-BOOT-02, IT-01, CL-11 | `PASS LOCAL`; `PASS HISTORICO WOKWI`; `PASS WOKWI FINAL` |
...
| RS-09: sensor timeout | Timeout de 350000 us y cierre fail-safe | `SafetyCore::handleSensorTimeout()` y `SAFE_CLOSE` | SAFE-FAULT-01 a SAFE-FAULT-04; CL-10 | `PASS LOCAL`; `PASS HISTORICO WOKWI`; `PASS WOKWI FINAL` |
...
| RF-15, RT-07, A05: timestamps | Fuente monotona y eventos separados para primera alta, confirmacion, comando, recepcion y aplicacion | `esp_timer_get_time()` y campos `TimingRecord` | A05-01 a A05-03; tests de parser y summary | `PASS LOCAL`; `PASS HISTORICO WOKWI`; `PASS WOKWI FINAL` |
| RT-03: confirmacion a recepcion <= 500000 us | `docs/analisis_temporal.md` define `T_ACTUATOR_RECEIVED - T_CRITICAL_CONFIRMED` | Instrumentacion en `TaskSafety` y `TaskActuator` | TT-01 a TT-06 | Campana anterior: Worst Observed Response Time 22504 us. Campana final: 22346 us, 25/25 PASS y margen 477654 us; `PASS WOKWI FINAL`. |
...
| M05: summary temporal determinista | Summary derivado de `wcet_observed.csv` sin editar evidencia fuente | `simulation/generate_wcet_summary.py` | `test_generate_wcet_summary.py` y `--check` | `PASS LOCAL`; CSV finales regenerados y verificados |
...
| --- | --- | --- | --- |
| CL-01 a CL-11 | Logica critica, startup, enclavamiento, rearme y timeout | Regresion funcional sobre el commit final | `PASS HISTORICO WOKWI`; `PASS WOKWI FINAL` |
| TT-01 | Zona 1 critica sostenida | `simulation/timing_test.yaml`; 5 corridas finales | `PASS WOKWI FINAL` |
| TT-02 | Zona 2 critica sostenida | `simulation/timing_zone2_test.yaml`; 5 corridas finales | `PASS WOKWI FINAL` |
| TT-03 | Ambas zonas criticas | `simulation/timing_both_test.yaml`; 5 corridas finales | `PASS WOKWI FINAL` |
| TT-04 | Diagnostico bajo carga | `simulation/timing_load_test.yaml`; 5 corridas finales | `PASS WOKWI FINAL` |
| TT-05 | Evento desplazado respecto al muestreo | `simulation/timing_phase_test.yaml`; 5 corridas finales | `PASS WOKWI FINAL` |
| TT-06 | Campana repetida normal y con carga | `simulation/run_timing_measurements.ps1`; 25 corridas finales | `PASS WOKWI FINAL` |

La campana final se identifica en `simulation/results/final_validation_manifest.json`.
No modifico la arquitectura critica ni el firmware de `src/` e `include/`.
